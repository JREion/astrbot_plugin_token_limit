...
> v0.6.2：Plugin Page 的“群聊个性化配置”新增“本群 token 节约策略”。勾选“仅通过 @bot 触发 LLM 回复”后，该群无论是否超限，都只允许 `@bot` 触发 LLM 回复；同一句消息同时包含 `@bot` 和唤醒词时按 `@bot` 处理。“重置”按钮只重置本群每日用量上限，不清除该勾选状态。
...
- 支持当日群聊内用户 token 用量统计，可选择单个群聊，查看刷新周期内 token 消耗最高的用户排行。
...
- 今日用户用量统计按当前 `refresh_time` 切分的统计周期展示，并在后台持久化保存近 48 小时的用户小时桶和请求归因数据；超过 48 小时的数据会被清理。
