...
- 提供历史 token 用量统计：群号加入限流列表后会从加入时刻开始按小时记录历史用量；即使之后从限流列表移除，插件运行时也会继续补齐并持久化保存。
- Plugin Page 的历史统计弹窗支持按群聊、近 24 小时、近 7 天、近一个月和历史总和查看柱状图。
...
历史统计使用 AstrBot 原生 `ProviderStat` token 记录作为数据源，插件只在自己的数据文件中保存每个群聊的小时级绝对用量桶和总量。同步时会覆盖最近一小段时间窗口内的小时桶，而不是累加增量，因此修改 `refresh_time` 不会导致历史数据重复统计。

历史数据文件位于 AstrBot 插件数据目录的 `history_usage.json`。新增群号的追踪起点是它首次出现在 `limited_groups` 配置中的时间点。
...
4. 打开 Plugin Page 查看限流群当前窗口内的用量进度，也可以点击“历史 token 用量统计”查看趋势图。
